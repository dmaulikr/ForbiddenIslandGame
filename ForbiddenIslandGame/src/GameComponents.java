...
class Player extends APiece  {
  ...
  //represents whether the player has won
  boolean won = false;
  //represents if the player is a scuba diver
  boolean canScuba = false;
  //represents whether the player is an Engineer
  boolean isEngineer = false;

  Player(ArrayList<ArrayList<Cell>> arr) {
    super(new RectangleImage(10, 10, OutlineMode.SOLID, Color.YELLOW), 0, 0);
  ...
  //moves the player
  void movePlayer(String ke, IList<Cell> l, String right, 
      String left, String up, String down)  {
  ...
  //moves the player if the player can Scuba dive
  void movePlayerScuba(String ke, IList<Cell> l, String right, 
      String left, String up, String down)  {
    IListIterator<Cell> it = new IListIterator<Cell>(l);
    Cell c2 = null;
    while (it.hasNext())  {
...
class Target extends APiece  {
  ...
  Target(ArrayList<ArrayList<Cell>> a)  {
    super(new EllipseImage(8, 8, OutlineMode.SOLID, Color.PINK), 0, 0);
...
class HelicopterPiece extends APiece  {
  ...
  HelicopterPiece(IList<Cell> board)  {
    super(new ScaleImage(new FromFileImage("src/helicopter.png"), 1), 0, 0);
...
//to represent a Scuba Piece
class Scuba extends APiece  {

  //determines whether a person can get the scuba
  boolean canGet = true; 

  Scuba(ArrayList<ArrayList<Cell>> a)  {
    super(new TextImage("S", 10, FontStyle.BOLD, Color.BLACK), 0, 0);
    Cell c = this.getNonOceanCell(a);
    this.x = c.x;
    this.y = c.y;
  }
...
//to represent an engineer ability
class EngineerPower extends APiece {

  EngineerPower(ArrayList<ArrayList<Cell>> a) {
    super(new ScaleImage(new FromFileImage("src/gear.png"), .05), 0, 0);
    Cell c = this.getNonOceanCell(a);
    this.x = c.x;
    this.y = c.y;
  }  
